...
設定した```SELECT```句がテーブルにアクセスする時に，どの程度の範囲にアクセスするかが表示される．大きな範囲にアクセスするSQLほど，想定実行時間が長くなる．

| 種類   | 条件                                                         | 事前に```fetch```するレコード数 | 補足                                                         |
| ------ | ------------------------------------------------------------ | ------------------------------- | ------------------------------------------------------------ |
| ALL    | ・インデックスを使用していない．                             | 全てのレコード                  | 全てのカラムを事前に```fetch```するため，実行時間が最も長く，改善する必要がある． |
| index  | ・インデックスを使用していない．                             | 全てのインデックスのレコード    |                                                              |
| range  | ・セカンダリインデックスを使用している．<br>・```WHERE```句に重複したカラム値，```IN```句，```BETWEEN```句を使用している． | 特定の複数カラム                |                                                              |
| ref    | ・セカンダリインデックスを使用している．<br>・```WHERE```句に重複しないカラム値 | 特定の複数カラム                |                                                              |

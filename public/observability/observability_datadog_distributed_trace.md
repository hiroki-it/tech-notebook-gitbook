...
分散トレースとログの統合タグ（```service```，```env```，```version```）に同じ値を割り当てると，分散トレース全体とログ間を紐づけることができる．

参考：https://docs.datadoghq.com/ja/tracing/connect_logs_and_traces/

#### ・スパンとログの紐づけ

分散トレースとログに同じトレースIDを割り当てると，スパンとログ間を紐づけることができる．これにより，そのログが，いずれのサービスで，またいつ発生したものかを確認できる．
...
分散トレースとコンテナのDockerLabelの統合タグ（```service```，```env```，```version```）に，同じ値を割り当てると，分散トレースとサーバ／コンテナのOSに関するメトリクスを紐づけることができる．

...
一定期間に発生した複数のデータポイントの集計値のこと．『平均』『合計』『最小／最大』『平方根』などを用いて，データポイントを集計する．
...
分析対象から得られる最小単位の数値データのこと．データポイントは，相対的に小さい粒度として定義されるだけであり，絶対的に小さい粒度というわけではないことに注意する．例えば，とある分析で一分ごとに対象が測定される場合，一分ごとに得られる数値データがデータポイントとなる．一方で，一時間ごとの測定の場合，一時間ごとに得られる数値データがデータポイントである．
...
データポイントの値だけが表示されたログのこと．

**＊例＊**

FluentBitが生成した非構造化ログを示す．
...
データポイントの項目名と値が表示されたログのこと．JSONが用いられることが多い．

**＊例＊**

FluentBitが生成した構造化ログを示す．
...
マイクロサービスアーキテクチャにおいて，サービスに渡った処理を表現するスパンのセットのこと．スパンを紐づけることによって，異なるサービスに分割されてしまった処理を，一繋ぎなものとして認識できるようになる．
...
サービスオーケストレーション方式がオーケストレーションの場合，上流／下流のサービス間でリクエスト／レスポンスが実行されるため，スパンの重なった逆三角形状の分散トレースが得られる．

#### ・コレオグラフィの場合

サービスオーケストレーション方式がコレオグラフィの場合，上流から下流のサービスに向かって一方通行的に処理が続くため，スパンの重ならない階段状の分散トレースを収集できる．
...
マイクロサービスアーキテクチャにおいて，特定のサービスで発生したデータポイントのセットのこと．データポイントには，特定のサービスの操作名（例：```web.request```，```db.query```），処理開始／終了のタイムスタンプ，エラーメッセージ，などをがある．

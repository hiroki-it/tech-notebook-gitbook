...
サービスオーケストレーション方式がオーケストレーションの場合，上流／下流のサービス間でリクエスト／レスポンスが実行されるため，スパンの重なった逆三角形状の分散トレースを収集できる．
...
マイクロサービスアーキテクチャにおいて，特定のサービスで発生したデータのセットのこと．データには，特定のサービスのスパン名（操作名），処理開始／終了のタイムスタンプ，エラーメッセージ，などをがある．

...
文字列データとして定義され，データポイントの値だけが表示されたログのこと．

```log
192.168.0.1 [2021-01-01 12:00:00] GET /users/1 200 
```

#### ・構造化ログ

多くの場合JSON型で定義され，データポイントの項目名と値が表示されたログのこと．
...
マイクロサービスアーキテクチャにおいて，複数のサービスから得られるスパンのセットのこと．スパンを紐づけることによって，異なるサービスを横断する処理を，一繋ぎなものとして認識できるようになる．

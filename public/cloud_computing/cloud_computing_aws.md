...
EC2インスタンスのEBSボリュームを```8```GBから```16```GBに拡張するとする。```lsblk```コマンドで現在のボリュームサイズを確認すると、ボリュームとパーティションがともに```8```GBである。

```bash
$ lsblk
NAME    MAJ:MIN RM SIZE RO TYPE MOUNTPOINT
xvda    202:0    0   8G  0 disk            # ボリューム
└─xvda1 202:1    0   8G  0 part /          # パーティション
```

コンソール画面から、EBSボリュームを```16```GBに変更する。ダウンタイムは発生しない。```lsblk```コマンドで確認すると、
...
DBインスタンスのフェイルオーバーを利用してダウンタイムを最小化する。非マルチAZ構成の場合、アプリケーションの向き先をプライマリーインスタンスにしたまま、変更対象のDBからリードレプリカを新しく作成し、これを更新した後に、リードレプリカを手動フェイルオーバーさせる。フェイルオーバー時に約```1```～```2```分のダウンタイムが発生するが、DBインスタンスの再起動よりも時間が短いため、相対的にダウンタイムを短縮できる。
...
マルチAZ構成の場合、アプリケーションの向き先をプライマリーインスタンスにしたまま、既存のリードレプリカを更新し、リードレプリカを自動フェイルオーバーさせる。フェイルオーバー時に約```1```～```2```分のダウンタイムが発生するが、DBインスタンスの再起動よりも時間が短いため、相対的にダウンタイムを短縮できる。

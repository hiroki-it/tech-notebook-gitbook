...
Aurora MySQLのアップグレードに伴うダウンタイムを計測する。踏み台サーバを経由してRDSに接続し、現在時刻を取得するSQLを送信する。この時、```for```文や```watch```コマンドを用いる。ただ、```watch```コマンドはデフォルトでインストールされていない可能性がある。平常アクセス時のも同時に実行することで、より正確なダウンタイムを取得するようにする。また、ヘルスチェックの時刻を正しくロギングできるように、ローカルPCから時刻を取得する。

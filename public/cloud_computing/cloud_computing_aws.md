...
| エンジンバージョン      | DBエンジンのバージョンを設定する。DBクラスター内の全てのDBインスタンスに適用される。 | ・```SELECT AURORA_VERSION()```を用いて、エンジンバージョンを確認できる。 |
...
踏み台サーバを経由してRDSに接続し、現在時刻を取得するSQLを送信する。この時、```for```文や```watch```コマンドを用いる。ただ、```watch```コマンドは標準でインストールされていない可能性がある。平常アクセス時のも同時に実行することで、より正確なダウンタイムを取得するようにする。また、ヘルスチェックの時刻を正しくロギングできるように、ローカルPCから時刻を取得する。

...
| eq_ref | ・クラスタインデックスを用いている。                       | 一つのカラム               | 一つのカラムしか```fetch```しないため、```JOIN```句を用いたアクセスの中で、実行時間が最も短い。 |
| const  | ・クラスタインデックスを用いている。<br>・```JOIN```句を用いていない。 | 一つのカラム               | 一つのカラムしか```fetch```しないため、実行時間が最も短い。  |

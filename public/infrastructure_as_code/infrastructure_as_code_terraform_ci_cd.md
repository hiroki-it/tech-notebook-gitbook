...
基本的には、プルリクを１つずつリリースするようにする。ただ、軽微なupdate処理が実行されるプルリクであれば、まとめてリリースしてもよい。もしリリース時に問題が起こった場合、インフラのバージョンのロールバックを行う必要がある。経験則で、create処理やdestroy処理よりもupdate処理の方がエラーが少ないため、ロールバックにもたつきにくい。プルリクを複数まとめてリリースすると、create処理やdestroy処理が実行されるロールバックに失敗する可能性が高くなる。

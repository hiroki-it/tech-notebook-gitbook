...
| ```x-datadog-trace-id```          | バックエンドがマイクロサービスアーキテクチャの場合に，収集できる分散トレースを紐付けるIDが割り当てられる． |
| ```x-datadog-parent-id```         | バックエンドがマイクロサービスアーキテクチャの場合に，分散トレースのルートスパンとして，```0```が割り当てられる． |
| ```x-datadog-origin```            | バックエンドがマイクロサービスアーキテクチャの場合に，分散トレースがAPMクオータに影響しないように，```synthetics-browser```が割り当てられる． |
| ```x-datadog-sampling-priority``` | バックエンドがマイクロサービスアーキテクチャの場合に，分散トレースが収集される優先度として，```1```が割り当てれる． |

...
『外部監視』ともいう。実際のユーザの一連の操作を模したリクエストをアプリケーションに送信し、レスポンスに関するメトリクスを収集した後、これを監視する。ユーザを模したリクエストを生成するという意味合いで、『合成』という。ユーザ視点で監視できる。
...
サーバ内にStatsDを常駐させ、アプリケーションでStatsDライブラリを用いると、メトリクスを収集できる。
...
サーバ／コンテナのOSに関するメトリクスを収集した後、これを監視する。メトリクスの例は以下に示す。

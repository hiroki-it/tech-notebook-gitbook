...
言語／フレームワーク／ツール、などに関して、専用のメトリクスを収集できるようになる。アプリケーションとして用いられる言語／フレームワークの場合、Datadogエージェントがインテグレーション処理を持つため、サーバ／コンテナへのインストールは不要である。Datedogエージェントがコンテナ／サーバで稼働する言語／フレームワーク／ツールを自動で認識してくれる。
